...
- `.env` / `.env.local` / `vendor/` / `node_modules/` / `var/` は提出対象に含めない
- PHP は 8.2 系または 8.3 系で確認する
- Node は v16.9.1、Yarn は 1.22.x を使用する
- FTP提出でビルド済みファイルが必要な場合は `yarn build` 後の `public_html/build/` を含める
- ソース提出の場合は `composer install` と `yarn install` で再現できる状態にする
...
DB接続設定とSMTP設定など聞かれるが、全てエンターでOK
...
本番環境では jpg,pngファイルから自動できに webp を生成して、`.htaccess` の設定で
webpが利用される様になっているので、ページ構築時に画像ファイルの圧縮は基本的に不要となっている
