...
プッシュ後に自動でビルドが走るが`webpack.config.js`の`cleanupOutputBeforeBuild()`
が有効になっていると、ビルド中のアクセスが500エラーになってしまうため`cleanupOutputBeforeBuild()`をキャンセルする。
...
特にリリース済みサイトでサブドメインを追加する場合は一時的にメインドメインの`.htaccess`
のSSLリダイレクトを外し、Basic認証がかかっている場合はこれも外して設定する必要がある
